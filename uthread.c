...
uthread_init(void)
{
  for(int i = 0; i < UTH_MAX_UTHREADS; i++){
    uthread_t *newUThread_ptr;
    newUThread_ptr = (uthread_t *)malloc(sizeof(uthread_t));
    uthreads[i] = *newUThread_ptr;
    newUThread_ptr->ut_state = UT_NO_STATE;
  }
...
{
  uthread_id_t newThread_id = uthread_alloc();
  if(newThread_id == -1){
    return -1;
  }
  uthread_t newThread = uthreads[newThread_id];
  uthread_makecontext(&newThread.ut_ctx, newThread.ut_stack, UTH_STACK_SIZE, func, arg1, arg2);
  memset(newThread.ut_link, 0, sizeof(list_link_t));
  newThread.ut_id = newThread_id;
  newThread.ut_errno = newThread.ut_has_exited = newThread.ut_exit = 0;
  newThread.ut_detached = 0;
  newThread.ut_waiter = NULL;
  uthread_nopreempt_on();
  newThread.ut_state = UT_RUNNABLE;
  // Possibly calls preempt_reset
  uthread_setprio(newThread_id, prio);
...
uthread_join(uthread_id_t uid, int *return_value)
{
  ...
  if(uid == uthread_self() || ut_curthr->ut_waiter->ut_id == uid){
    ...
    threadToWaitFor.ut_detached = 0;
    uthread_block();
    *return_value = threadToWaitFor.ut_exit;
    result = uthread_detach(uid);
  }
  uthread_nopreempt_off();
  return result;
}
...
uthread_detach(uthread_id_t uid)
{
  ...
  if(thread_to_detach.ut_has_exited){
